public class sudoku {
	...
	static boolean isValid(int[][] matrix, int x, int y) { // verifie si une
		...
		int xi = (x / n) * n; // coordonnées de la case en haut a gauche du bloc
								// auquel appartient la case traitée
		int yi = (y / n) * n;
		...
		return true; // si aucun des testes précédents n'a retourné false, la
						// case est valide
	}

	static boolean solve(int[][] matrix, int x, int y) { // résoud le sudoku
															// passé en
															// cherchant une
															// solution par un
															// algorithme de
